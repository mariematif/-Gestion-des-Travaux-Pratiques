...
public class EtudiantService implements IDao<Etudiant> {
    ...
    public Iterable<Etudiant> findByTP(TP selectedTP) {
        List<Etudiant> etudiants = new ArrayList<>();
        String req = "SELECT e.* FROM Etudiant e "
                + "JOIN ParticipationTP p ON e.id = p.etudiant_id "
                + "WHERE p.tp_id = ?";
        ...
        try {
            ...
            System.out.println("Erreur lors de la récupération des étudiants par TP : " + ex.getMessage());
        }
    ...
    try {
        ...
        ps.setString(2, "%" + prenom + "%"); // Recherche partielle sur le prénom
        ResultSet rs = ps.executeQuery();
        ...
        System.out.println("Erreur lors de la recherche de l'étudiant par nom ou prénom : " + ex.getMessage());
    }
